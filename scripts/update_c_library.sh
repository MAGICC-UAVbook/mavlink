...
# Usage, for example:
# cd ~/src
# git clone https://github.com/magicc-uavbook/mavlink.git
# cd mavlink
...
generate_headers MAGICC
echo -e "\0033[34mFinished generating c headers\0033[0m\n"

# git add and git commit in local c_library repository
cd $CLIBRARY_PATH
git add --all :/ || exit 1
COMMIT_MESSAGE="autogenerated headers for rev https://github.com/magicc-uavbook/mavlink/tree/"$MAVLINK_GITHASH
